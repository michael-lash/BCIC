...
load('~/ARIC/experiment/benchmark/normBenchDSetsAllUpdated.mat');
load('~/ARIC/experiment/benchmark/logistic_model_multiple_Bench.mat');
...
%     indirectx = tempmatrix1'./tempmatrix2;
    ...
    for i=1:length(var.B_list) 
        var.B=var.B_list(i);
        [x obj]=sens_kernel(var);
        Prob(useid,i+1)=1./(1+exp(-LogisticModel{kFoldInd(useid)}.LogisticPara(1)-min(obj)*LogisticModel{kFoldInd(useid)}.LogisticPara(2)));
        [useid i]
    end
end
%save('recommend_output_linear.mat','Prob');
save('~/ARIC/experiment/benchmark/data_output/sens_recommend_output_kernel_setting1.mat','Prob');
